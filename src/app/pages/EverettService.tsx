...
import { C, SERVICES, TESTIMONIALS } from '../data'
...
export default function EverettService() {
  const { slug } = useParams<{ slug: string }>()
  const seo = getSeoService(slug ?? '')
  const service = SERVICES.find(s => s.slug === slug)

  if (!seo || !service) return <Navigate to="/services" replace />

  const otherServices = SERVICES.filter(s => s.slug !== slug).slice(0, 4)
  const reviews = TESTIMONIALS.filter(t => t.service === service.title || t.stars === 5).slice(0, 3)
    ...
    <div style={{ background: C.dark }}>
      ...
      <section style={{ position: 'relative', minHeight: '480px', display: 'flex', alignItems: 'flex-end', overflow: 'hidden', paddingTop: '72px' }}>
        <div style={{ position: 'absolute', inset: 0 }}>
          <img src={seo.heroImg} alt={seo.h1} style={{ width: '100%', height: '100%', objectFit: 'cover', filter: 'brightness(0.3)' }} />
        ...
        <div style={{ position: 'relative', maxWidth: '1200px', margin: '0 auto', padding: '80px 28px 64px', width: '100%' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '20px', flexWrap: 'wrap' }}>
            ...
            <Link to="/service-areas" style={{ fontSize: '12px', color: C.dim, textDecoration: 'none' }}>Everett, WA</Link>
            <span style={{ color: C.dim2 }}>›</span>
            <span style={{ fontSize: '12px', color: C.cyan }}>{service.title}</span>
          </div>
          <p className="fade-up" style={{ fontSize: '11px', letterSpacing: '0.35em', textTransform: 'uppercase', color: C.cyan, marginBottom: '14px', fontWeight: 600 }}>Everett, WA · Snohomish County</p>
          <h1 className="fade-up-1" style={{ fontFamily: "'Barlow Condensed',system-ui,sans-serif", fontWeight: 800, fontSize: 'clamp(38px,6vw,78px)', textTransform: 'uppercase', lineHeight: 0.92, letterSpacing: '-0.01em', maxWidth: '800px' }}>
            {seo.h1.split(' in ')[0]}<br /><span className="cyan-shimmer">in Everett, WA</span>
          </h1>
          ...
          <div className="fade-up-3" style={{ display: 'flex', gap: '14px', marginTop: '32px', flexWrap: 'wrap' }}>
            <BtnPrimary href="/contact">📅 Book in Everett</BtnPrimary>
      ...
      <div style={{ background: C.dark2, borderBottom: `1px solid ${C.border}`, padding: '18px 28px' }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto', display: 'flex', gap: '32px', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'center' }}>
          {[
            ...
            { icon: '📍', text: 'Everett, WA Based' },
            { icon: '💬', text: 'Free Quotes' },
          ].map((item, i) => (
            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '13px', color: C.dim, fontWeight: 400 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 380px', gap: '64px', alignItems: 'start' }} className="two-col">
            ...
            <div>
              <Reveal>
                ...
                <p style={{ fontSize: '16px', color: C.dim, lineHeight: 1.85, fontWeight: 300, marginBottom: '28px' }}>{seo.intro}</p>
              ...
              <Reveal>
                <h2 style={{ fontFamily: "'Barlow Condensed',system-ui,sans-serif", fontWeight: 800, fontSize: 'clamp(22px,3vw,38px)', textTransform: 'uppercase', margin: '56px 0 20px' }}>
                  Why Everett Drivers <Shimmer>Choose Us</Shimmer>
                </h2>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                  {seo.whyLocal.map((reason, i) => (
                    <div key={i} style={{ display: 'flex', gap: '12px', alignItems: 'flex-start' }}>
                      ...
                      <p style={{ fontSize: '14px', color: C.dim, lineHeight: 1.7, fontWeight: 300 }}>{reason}</p>
                    </div>
                  ))}
                </div>
              </Reveal>
            ...
            <div style={{ position: 'sticky', top: '120px' }}>
              <Reveal>
                <div style={{ background: C.surface, border: `1px solid ${C.borderB}`, padding: '32px 28px' }}>
                  <div style={{ position: 'relative', top: 0, left: 0, right: 0, height: '2px', background: `linear-gradient(90deg, ${C.blue}, ${C.cyan})`, margin: '-32px -28px 28px' }} />
                  <p style={{ fontSize: '11px', letterSpacing: '0.2em', textTransform: 'uppercase', color: C.cyan, marginBottom: '8px', fontWeight: 600 }}>Book in Everett, WA</p>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', marginBottom: '28px' }}>
                    {[
                      ...
                      { icon: '📍', val: 'Everett & Snohomish County', label: 'Service Area' },
                    ].map((row, i) => (
                  ...
                  <a href="tel:4252300876" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', marginTop: '12px', padding: '12px', border: `1px solid ${C.border}`, color: C.text, textDecoration: 'none', fontSize: '13px', fontWeight: 600, transition: 'border-color 0.2s' }}
                  ...
                  <div style={{ marginTop: '20px', padding: '16px', background: C.panel, textAlign: 'center' }}>
                    ...
                    <div style={{ fontSize: '12px', color: C.dim, lineHeight: 1.7 }}>
                      📍 11615 25th Ave SE, Everett WA 98208<br />
                      📍 5158 82nd Pl NE, Marysville WA 98270
                    </div>
                  </div>
                </div>
              </Reveal>
        ...
        <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 28px' }}>
          <Reveal>
            <h2 style={{ fontFamily: "'Barlow Condensed',system-ui,sans-serif", fontWeight: 800, fontSize: 'clamp(26px,3.5vw,44px)', textTransform: 'uppercase', marginBottom: '40px' }}>
              What Everett Clients <Shimmer>Say</Shimmer>
            </h2>
          </Reveal>
        ...
        <div style={{ maxWidth: '860px', margin: '0 auto', padding: '0 28px' }}>
          <Reveal>
            ...
            <p style={{ fontSize: '15px', color: C.dim, marginBottom: '40px', fontWeight: 300 }}>
              Common questions about {service.title.toLowerCase()} in Everett, WA.
            </p>
          </Reveal>
          {seo.faqs.map((faq, i) => (
        ...
        <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 28px' }}>
          <Reveal>
            ...
            <h2 style={{ fontFamily: "'Barlow Condensed',system-ui,sans-serif", fontWeight: 800, fontSize: 'clamp(22px,3vw,38px)', textTransform: 'uppercase', marginBottom: '24px' }}>
              {service.title} Near Everett, WA
            </h2>
            <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
              {seo.nearbyAreas.map((area, i) => (
                <Link key={i} to="/service-areas"
            ...
            <p style={{ fontSize: '14px', color: C.dim, marginTop: '20px', fontWeight: 300, lineHeight: 1.7 }}>
              We provide {service.title.toLowerCase()} throughout Snohomish County and the greater Seattle area. If you're within 30 miles of Everett, we can come to you. <Link to="/service-areas" style={{ color: C.cyan, textDecoration: 'none' }}>View all service areas →</Link>
            </p>
          </Reveal>
        </div>
      </section>
        ...
        <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 28px' }}>
          <Reveal>
            <h2 style={{ fontFamily: "'Barlow Condensed',system-ui,sans-serif", fontWeight: 800, fontSize: 'clamp(26px,3.5vw,44px)', textTransform: 'uppercase', marginBottom: '36px' }}>
              More Services in <Shimmer>Everett</Shimmer>
            </h2>
          </Reveal>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: '2px', background: C.border }} className="pkg-three">
            {otherServices.map((s, i) => (
              <Reveal key={s.slug} delay={i * 0.06}>
                <Link to={`/everett/${s.slug}`} style={{ textDecoration: 'none', display: 'block', background: C.surface, padding: '24px 20px', transition: 'background 0.2s', height: '100%' }}
